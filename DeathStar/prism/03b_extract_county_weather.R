# ============================================================
# Script: 03b_extract_county_weather.R
# Purpose: Extract area-weighted mean weather values from PRISM
#          daily rasters for each Texas county polygon.
# Inputs:  prism_raw/ (BIL rasters), intermediate_data/county_shapefiles/tx_counties_2024.gpkg
# Outputs: intermediate_data/extracted_county/{var}_{year}.csv
# Author: EK  Date: 2026-08-07
#
# Strategy: identical to 03_extract_cz_weather.R but keyed on
# county (fips) polygons instead of CZ polygons. Stack a full
# year of rasters per variable into a SpatRaster, crop once to
# Texas, then call exact_extract once. This amortizes per-call
# overhead (CRS transform, spatial index build) across all days.
#
...
SHP_PATH      <- file.path(cfg$counties$shp_dir,
                           "tx_counties_2024.gpkg")
EXTRACTED_DIR <- cfg$output$extracted_county_dir

fs::dir_create(EXTRACTED_DIR, recurse = TRUE)

cat(glue("
╔══════════════════════════════════════════════════════╗
║  Step 3b: Zonal Extraction — PRISM -> County Polygons ║
╚══════════════════════════════════════════════════════╝
...
# ── 1. Load Texas county shapefile ─────────────────────────
if (!file.exists(SHP_PATH)) {
  stop(glue("Shapefile not found: {SHP_PATH}\nRun 02b_get_counties.py first."))
}

county_sf <- sf::st_read(SHP_PATH, quiet = TRUE) |> sf::st_transform(4326)
cat(glue("  Loaded {nrow(county_sf)} Texas counties.\n\n"))

if (nrow(county_sf) != 254) {
  stop(glue("Expected 254 Texas counties, found {nrow(county_sf)}. Aborting (§1 tripwire)."))
}

# Pre-compute TX bounding box for cropping (used every year)
tx_bbox <- sf::st_bbox(county_sf)
...
# Returns a tidy data frame: fips | date | <var>
extract_year <- function(var, yr, file_tbl, county_nad83, tx_ext, out_dir) {
  ...
  tryCatch({
    ...
    # exact_extract returns a data frame: nrow = n_counties, cols = mean.<date>
    ...
    # Reshape to long: fips | date | <var>
    result <- vals |>
      as.data.frame() |>
      mutate(fips = county_nad83$fips) |>
      tidyr::pivot_longer(
        cols      = -fips,
        names_to  = "date",
        values_to = var
...
# ── 4. Pre-transform county polygons to raster CRS (once) ─
# All PRISM rasters use NAD83 geographic CRS. Transform county
# polygons once here rather than inside the extraction loop.
cat("  Pre-transforming county polygons to NAD83...\n")
...
county_nad83 <- sf::st_transform(county_sf, crs = raster_crs)
rm(r_sample)
...
for (var in VARIABLES) {
  ...
  for (yr in years) {
    extract_year(var, yr, file_tbl, county_nad83, tx_ext, EXTRACTED_DIR)
  }
}
...
cat("\nDone. Proceed to 04b_build_county_panel.py\n")
